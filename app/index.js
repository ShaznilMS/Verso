...
import { GeneratePosts, GetAuthentication, GetPosts, SignIn, SignOut, SignUp, VerifyAuthentication, AddPosts, DeletePosts, LikePost, CommentPost, CountPosts, CountPostsComments, CountPostsLikes } from './Settings/index.mjs'
...
function Item({ Name, Time, Category, Content, Quote, onDelete }) {
    ...
    return (
        <TouchableWithoutFeedback onLongPress={() => { SHOW(); Vibration.vibrate(40) }} onPress={() => HIDE()} >
            <View style={{ width: '100%', minWidth: '100%', minHeight: 100, backgroundColor: '#fff', gap: 5, flexDirection: 'row', overflow: 'hidden' }}>
...
function ShowPost({ isVisible = false, onPressButton1, onPressButton2 }) {
    ...
    function Delete(index) {
        setRefreshing(true)
        DeletePosts(Object.keys(data)[index])
        Data()
        setRefreshing(false)
            ...
            <View style={styles.container}>
                ...
                >
                    <View style={{flex: 1, gap: 10}}>
                        ...
                        <Button label={'Send Comment'} method={() => CommentPost('-O2VtogPpTM7KUkTBeSR', 'Shaznil Mussagy Sulemane', 'Opha')} />
                        <Button label={'Like Post'} method={() => LikePost('-O2VtogPpTM7KUkTBeSR', 'Shaznil')} />
                        <Button label={'Count Comments'} method={() => CountPostsComments('-O2VtogPpTM7KUkTBeSR')} />
                        <Button label={'Count Likes'} method={() => CountPostsLikes('-O2VtogPpTM7KUkTBeSR')} />
